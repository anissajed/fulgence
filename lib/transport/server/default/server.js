import http from 'http';
import {REQ_MAX_SIZE_BYTES} from "../../../constants.js";

/** @typedef {import("./server.types.js").ServerWithPlugin} ServerWithPlugin */
/** @type ServerWithPlugin */
export const withPlugin = (plugin, baseServer) => {
  const server = (input) => baseServer({
    ...input,
    opts: {
      ...(input.opts || {}),
      onRequest: async (data, req) => {
        let res = data;
        if (plugin.onRequest) {
          res = plugin.onRequest(res, req);
        }
        if (input.opts?.onRequest) {
          res = input.onRequest(res, req);
        }
        return res;
      },
      beforeResponseSent: async (data) => {
        let res = data;
        if (input.opts?.beforeResponseSent) {
          res = input.opts.beforeResponseSent(res);
        }
        if (plugin.beforeResponseSent) {
          res = plugin.beforeResponseSent(res);
        }
        return res;
      },
    },
  });

  return server;
}

const requestBody = (req, req_max_size_bytes = 0) => new Promise((resolve, reject) => {
  let body = '';
  req.on('data', chunk => {
    body += chunk.toString();

    if (req_max_size_bytes && body.length > req_max_size_bytes) {
      const err = new Error("Request body too large");
      reject(err);
    }
  });
  req.on('end', () => {
    let body_obj = null;
    try {
      body_obj = JSON.parse(body);
    } catch (error) {
      return reject(new Error("Request body is not valid JSON"));
    }
    resolve(body_obj);
  });
});

const rootPostRequestController = async ({
  req,
  res,
  api,
  req_max_size_bytes = REQ_MAX_SIZE_BYTES,
  onRequest = (body, req) => body,
  beforeResponseSent = (data) => data,
}) => {
  const task_name = req.url.replace(/^\//, "");
  const task = api[task_name];
  let input = await requestBody(req, req_max_size_bytes);
  input = await onRequest(input, req);

  let result = await task(input);
  result = await beforeResponseSent(result);

  res.statusCode = 200;
  res.setHeader('Content-Type', 'application/json');
  const result_serialized = JSON.stringify(result);
  res.end(result_serialized);
}

const rootPostRequestControllerWrapper = async ({
  req,
  res,
  api,
  ...controller_opts
}) => {
  try {
    await rootPostRequestController({
      req,
      res,
      api,
      ...controller_opts,
    });
  } catch (error) {
    res.statusCode = 200;
    const err_label = error.toString ? error.toString() : "Unknown error";
    const err_body = JSON.stringify({error: err_label});
    res.end(err_body);
  }
};

const handleNotFound = (req, res) => {
  res.statusCode = 404;
  res.end();
}

/** @typedef {import("./server.types.js").DefaultServer} DefaultServer */
/** @type DefaultServer */
export const server = async ({
  name,
  api,
  opts: {
    port = 3000,
    onReady = ({api}) => {},
    ...controller_opts
  },
}) => {
  const tasks_urls = Object.keys(api).map((task_name) => "/" + task_name);
  const server = http.createServer(async (req, res) => {
    if (req.method != "POST" || !tasks_urls.includes(req.url)) {
      return handleNotFound(req, res);
    }

    await rootPostRequestControllerWrapper({req, res, api, ...controller_opts});
  });

  server.listen(port, () => onReady({api}));

  return server;
}
