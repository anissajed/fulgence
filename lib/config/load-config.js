...
/** @type import("./load-config.types.js").BuildConfig */
export const buildConfig = ({raw_config, config_path = ""}) => {
  ...
  const basepath = config_path ? path.dirname(config_path) : "/";
...
/** @type import("./load-config.types.js").GetRawConfig */
export const getRawConfig = async (opts) => {
  if ("config" in opts && opts.config) {
    return opts.config;
  }

  if ("config_path" in opts && opts.config_path) {
    const raw_config = await defaultFileLoader(opts.config_path);
    return raw_config;
  }

  throw new Error(
    "At least one of 'config', 'config_path' fields is needed; "
    + "for config_path, it must be a valid path."
  );
}
