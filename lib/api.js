...
const localTaskCaller = async ({
  ...
  const callTask = onDoTask({task, api, local: true});
  return callTask;
};
...
const remoteApi = async ({config, onInitTask, onDoTask}) => {
  ...
  await makeApi(config.tasks, async ({name, task_config}) => {
    ...
    const default_export = clientFactory({name, url: task_config.url});
    const doRequest = await onInitTask({module: {default: default_export}, api});
    const taskFactory = (extra) => (input, _) => doRequest(input, extra);
    const callTask = (input, extra) => onDoTask({task: taskFactory(extra), input, api});
    api[name] = callTask;
    return callTask;
  });
